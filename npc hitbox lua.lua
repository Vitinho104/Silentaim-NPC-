...
getgenv().ClientPullMode = "RenderStepped" -- "RenderStepped", "Heartbeat", "Stepped", "Simulation"
getgenv().ClientPullAggressive = false -- NOVO: Modo agressivo para puxar QUALQUER NPC
...
PullGroup:AddToggle('ClientPullAggressive', {
    ...
    Callback = function(v) 
        ...
        Library:Notify(v and '⚠️ CLIENT PULL AGRESSIVO ATIVADO - Puxando TODOS os NPCs' or 'Client Pull normal ativado', 2)
    end
})
...
PullInfo:AddLabel('Funciona apenas visualmente no cliente.')
PullInfo:AddLabel('Modo Agressivo: Puxa até NPCs sem tags!')
...
-- ==========================================
-- [[ 9. FUNÇÃO CLIENT PULL (TODOS JUNTOS, DISTANTES) - AGORA COM MODO AGRESSIVO ]]
-- ==========================================
local function pullNPCs()
    ...
    -- Ponto alvo: na frente do jogador, na distância definida
    ...
    if aggressive then
        -- MODO AGRESSIVO: Puxa QUALQUER modelo que pareça ser um NPC
        ...
        -- MODO NORMAL: Usa o cache de NPCs
        ...
        -- Também puxa NPCs novos que podem não estar no cache ainda
        for _, obj in pairs(workspace:GetDescendants()) do
            if obj:IsA("Model") and obj:FindFirstChild("Humanoid") and obj:FindFirstChild("HumanoidRootPart") then
                if not IsPlayerCharacter(obj) and IsNPC(obj) then
                    local found = false
                    for _, cached in pairs(npcCache) do
...
-- Heartbeat para Heartbeat e Simulation (com throttles diferentes)
RunService.Heartbeat:Connect(function(dt)
    ...
    -- Simulation mode (0.03s throttle - mais rápido que Heartbeat)
    ...
    -- Continua com as outras funções do Heartbeat
...
Library:Notify('Hitbox Expander v7 (Client Pull Agressivo) carregado com sucesso!', 5)