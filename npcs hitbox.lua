...
-- ==========================================
-- [[ 9. FUNÇÃO CLIENT PULL (TODOS JUNTOS, DISTANTES) ]]
-- ==========================================
local function pullNPCs()
    ...
    for _, npc in pairs(workspace:GetDescendants()) do
        if npc:IsA("Model") and npc:FindFirstChild("Humanoid") and npc:FindFirstChild("HumanoidRootPart") then
            if not IsPlayerCharacter(npc) then
                local npcRoot = npc:FindFirstChild("HumanoidRootPart")
                if npcRoot then
                    -- Puxa TODOS os NPCs para o MESMO ponto (na frente do jogador)
...
Library:Notify('Hitbox Expander v6 (Auto-Respawn & Anti-Desync) + Client Pull (TODOS JUNTOS, DISTANTES) carregado com sucesso!', 5)