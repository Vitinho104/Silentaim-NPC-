...
local Window = Library:CreateWindow({
    Title = "🎯 Silent Aim v3.1 (Anti-Lag + Anti-Kick)",
...
TabNPC:CreateToggle("Modo Agressivo (Otimizado)", function(Value)
    ...
    LastCacheUpdate = 0; UpdateCaches()
    if Value then Library:Notification({Title = "⚠️ MODO AGRESSIVO", Text = "ATIVADO - Anti-Lag Ligado", Duration = 2, Type = "Warning"}) end
end, false)
...
TabInfo:CreateLabel("Versão: 3.1 Anti-Lag + Anti-Kick")
TabInfo:CreateLabel("✅ Proteção Anti-Kick Ativada Automaticamente")
TabInfo:CreateButton("Limpar UI", function() CleanupUIOnly() end)

Library:Notification({
    Title = "🎯 SILENT AIM v3.1",
    Text = "Anti-Lag e Anti-Kick ativados!",
    Duration = 4,
    Type = "Success"
})
