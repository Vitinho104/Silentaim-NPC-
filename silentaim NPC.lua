...
local function UpdateTargetInfo()
    ...
    if CurrentTarget and TargetPart then
        ...
        if onScreen then
            ...
            -- Chance de acerto
            if _G.ShowHitChance then
                -- Gerar chance de acerto aleatória baseada em _G.HitChance
                local randomChance = math.random(1, 100)
                local hitStatus = randomChance <= _G.HitChance and "✅ Hit!" or "❌ Miss"
                table.insert(infoLines, string.format("%s (%d%%)", hitStatus, _G.HitChance))
            end
...
--// HOOKS ATUALIZADOS COM TELEPORTE DE BALA
if not oldNamecall then
    oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if _G.BulletTeleport and (method == "FireServer" or method == "InvokeServer") and typeof(self) == "Instance" then
            ...
            local bulletFunctions = {"fire", "shoot", "bullet", "ammo", "projectile", "missile", "rocket"}
            ...
            if isBulletFunction then
                ...
                for i, arg in pairs(args) do
                    if typeof(arg) == "Vector3" then
                        ...
                        local newOrigin, newDirection = TeleportBulletToTarget(arg, Vector3.new(0, 0, 1), selfName)
                        
                        -- Modificar argumentos
                        local newArgs = {}
                        for j, oldArg in pairs(args) do
                            if j == i then
                                newArgs[j] = newOrigin
                            else
                                newArgs[j] = oldArg
                        ...
                        -- Adicionar direção se necessário
                        if #newArgs == i then
                            table.insert(newArgs, newDirection)
                        else
                            newArgs[i + 1] = newDirection
                        end
                        
                        return oldNamecall(self, unpack(newArgs))
                    elseif typeof(arg) == "Ray" then
                        -- Teleportar raio da bala
                        local newOrigin, newDirection = TeleportBulletToTarget(arg.Origin, arg.Direction, selfName)
                        local newRay = Ray.new(newOrigin, newDirection)
...
--// ATUALIZAR CÍRCULO, INFORMAÇÕES E HIGHLIGHT
...
local function CreateCompactMobileUI()
    ...
    HeaderText.Text = "🎯 SILENT AIM v2.5"
    ...
    local Tabs = {"Geral", "Config", "Aim", "Team", "Extra", "Info"}
    ...
    HitChanceText.Text = "Hit Chance: 95%"
    ...
    --// CONTEÚDO DA ABA INFO (Controles individuais)
    ...
    local function UpdateHitChance(value)
        _G.HitChance = math.clamp(value, 1, 100)
        HitChanceText.Text = "Hit Chance: " .. math.floor(_G.HitChance) .. "%"
        HitChanceSliderFill.Size = UDim2.new(_G.HitChance / 100, 0, 1, 0)
    end
    
    --// SLIDER DO FOV CORRIGIDO (não atualizar quando clicar fora)
        ...
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            local function onInputChanged(input)
                if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseMovement then
                    -- Garantir que estamos dentro dos limites do slider
...
task.spawn(function()
    ...
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "🎯 SILENT AIM v2.5",
        Text = "Novos recursos adicionados!\n• Highlight no alvo\n• Controles individuais de info\n• Nova aba 'Info'",
        Duration = 3,
        Icon = "rbxassetid://4483345998"
    })
...
║                 VERSION 2.5                  ║
║                                              ║
║  • Highlight no alvo (brilho colorido)       ║
║  • Controles individuais de informações:     ║
║     - Nome/Tipo do alvo                      ║
║     - HP do alvo                             ║
║     - Distância                              ║
║     - Chance de acerto                       ║
║  • Nova aba 'Info' com controles detalhados  ║
